...
1. 冷启动测量与 I/O 追踪建议在同一设备、同一电量/温度条件下进行，以降低噪声。
2. 涉及 drop_caches 与 ftrace 的脚本需 root 权限。
3. 生成的 CSV 与 PNG 文件建议统一保存并标注测试日期，避免覆盖。

如需扩展到其他应用，请将脚本中的包名与启动 Activity 替换为目标应用即可。
