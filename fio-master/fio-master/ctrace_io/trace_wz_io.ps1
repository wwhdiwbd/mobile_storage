Write-Host "=== WZ IO Trace ===" -ForegroundColor Cyan
...
function Prepare-ColdStart {
    ...
    # Force stop app
    adb shell am force-stop com.tencent.tmgp.sgame 2>$null
    
    # Clear app cache
    Write-Host "  - Clearing app cache" -ForegroundColor Gray
    adb shell "rm -rf /data/data/com.tencent.tmgp.sgame/cache/*" 2>$null
    adb shell "rm -rf /data/data/com.tencent.tmgp.sgame/code_cache/*" 2>$null
...
Write-Host "2. Launching WZ..." -ForegroundColor Yellow
$result = adb shell "am start -W -n com.tencent.tmgp.sgame/.SGameActivity" 2>$null
...
adb shell "atrace --async_stop -z -o /data/local/tmp/wz_io_trace_without_pagecache.z"

Write-Host "5. Pulling trace file..." -ForegroundColor Yellow
adb pull /data/local/tmp/wz_io_trace_without_pagecache.z
...
adb shell am force-stop com.tencent.tmgp.sgame 2>$null
adb shell "am start -n com.tencent.tmgp.sgame/.SGameActivity" 2>$null
Start-Sleep -Seconds 5
adb shell am force-stop com.tencent.tmgp.sgame 2>$null
...
Write-Host "2. Launching WZ..." -ForegroundColor Yellow
$result = adb shell "am start -W -n com.tencent.tmgp.sgame/.SGameActivity"
Write-Host $result -ForegroundColor Green
...
Write-Host "4. Stopping trace..." -ForegroundColor Yellow
adb shell "atrace --async_stop -z -o /data/local/tmp/wz_io_trace_with_pagecache.z"

adb pull /data/local/tmp/wz_io_trace_with_pagecache.z
...
if ($file1 -and $file2) {
    ...
    Write-Host "  1. WITHOUT PageCache: .\wz_io_trace_without_pagecache.z ($size1MB MB)" -ForegroundColor White
    Write-Host "  2. WITH PageCache:    .\wz_io_trace_with_pagecache.z ($size2MB MB)" -ForegroundColor White
    ...
    if (-not $file1) { Write-Host "  Missing: wz_io_trace_without_pagecache.z" -ForegroundColor Red }
    if (-not $file2) { Write-Host "  Missing: wz_io_trace_with_pagecache.z" -ForegroundColor Red }
    Write-Host "Then drag in: .\wz_io_trace_with_pagecache.z" -ForegroundColor White
} else {
